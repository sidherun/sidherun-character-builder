...
Language references (Quin'dhel/Elven naming primitives, etc.) live in [`../languages/`](../languages/README.md) — same golden-page handling, separate directory.
...
1. **Assemble the book:** concatenate the chapter files in numeric order (`00-overview.md` → `20-world-southern-shores.md`; skip `flavor/` or append it as a closing section; optionally append `../languages/` docs as a naming appendix). Prepend a cover block: title, `rules/VERSION`, "rulings as of" date (last `FIDELITY-NOTES.md` ruling), and a generated table of contents.
