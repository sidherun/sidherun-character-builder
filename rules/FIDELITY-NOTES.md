...
- §1.13 **RESOLVED** (Ed, 2026-07-11) — **Attack bonus is non-stacking**: a weapon attack uses the weapon-skill value when the character is skilled with the weapon, otherwise the governing attribute — never both. Situational and misc bonuses (positioning, affinities, buffs, items) still add on top. The Martinuk example (ch. 7 essentials) has been **corrected in text** to the ruled math — attack bonus 30+20+10 = 60, roll 150, margin 80, same double-damage outcome — with the original summed version preserved verbatim in the ruling note (the §1.1/§1.3 precedent); the Attacker worksheets' *Ability bonus* and *Skill/Trait/Power bonus* columns are alternatives for the weapon roll, not addends. Matches the app (#152) and the per-weapon data rulings (#203); the campaign has been played non-stacking since kickoff. App tracker: #264.
