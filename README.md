...
- **GM Screen** — a coordination dashboard (opened from the Roster) showing every character's HP / Mana / Story Points with inline +/− adjusters and an **Open** link into full Play Mode. For cloud-synced characters it updates **live** — a player's HP/notes change on their own device appears on the GM's screen in real time — and a **Live Rolls** feed streams every player's dice rolls as they happen.
...
- **Dice rolls** (`dice.js` + `rollActions.js`) — two resolution shapes on a d100:
  - **Skills & attacks** roll `d100 + a single modifier` and **display the total**; the player reads it aloud and the GM adjudicates against difficulty/defense verbally (like D&D Beyond). The attack modifier is the weapon's **skill _or_ attribute value — non-stacking**, chosen by the weapon's explicit **`usesSkill`** flag (toggle it in the Combat editor; a skilled weapon uses its skill value even when that value is 0), computed by `weaponModifier()` and shown consistently in Play Mode, the Review sheet, the Combat editor, and the printable sheet. Legacy characters saved before the flag existed are migrated on import by inferring `usesSkill` from a nonzero skill bonus.
  ...
  - **3D dice animation + sound** — a tap tumbles physical percentile dice (a tens die 00–90 + a ones die 0–9) that land on the rolled number, with a continuous rolling rattle (Web Audio) that thins out as the dice slow and ends on a settle clack. Built on `@3d-dice/dice-box-threejs`, lazy-loaded on the first roll (its own bundle chunk, so app start is untouched); the exact landing value comes from `diceNotation.js`. Header toggles for 🎲 animation and 🔊 sound (persisted); animation auto-off under `prefers-reduced-motion`; sound is our own layer (the engine's built-in audio is iOS-fragile). The result banner + shared feed are the source of truth, so a device that can't render still shows the number instantly.
